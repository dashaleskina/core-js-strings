...
function removeFirstOccurrences(str, value) {
  const point = str.indexOf(value);
  if (point === -1) return str;
  return str.slice(0, point) + str.slice(point + value.length);
}
...
function removeLastOccurrences(str, value) {
  const point = str.lastIndexOf(value);
  if (point === -1) return str;
  return str.slice(0, point) + str.slice(point + value.length);
}
...
function countVowels(str) {
  ...
  return str
    .split('')
    .map((item) => vowels.includes(item.toUpperCase()))
    .reduce((acc, curr) => acc + curr, 0);
}
...
function isPalindrome(str) {
  const updateStr = str.toUpperCase().replace(/[^A-Z]/g, '');
...
function findLongestWord(sentence) {
  const sortArray = sentence.split(' ').sort((a, b) => a.length - b.length);
  const maxStrLength = sortArray[sortArray.length - 1].length;
  const firstEnter = sortArray.find((item) => item.length === maxStrLength);
  return firstEnter;
}
...
function encodeToRot13(str) {
  const strCodes = str.split('').map((letter) => letter.charCodeAt());
  const rot13Str = strCodes.map((item) => {
    if ((item > 64 && item < 78) || (item >= 97 && item < 110)) {
      return String.fromCharCode(item + 13);
    }
    if ((item >= 78 && item < 98) || (item > 100 && item <= 122)) {
      return String.fromCharCode(item - 13);
    }
    return String.fromCharCode(item);
  });
  return rot13Str.join('');
}
